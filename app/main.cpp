...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < vio_dataset->get_image_timestamps().size() - 100; i += 10) {
    ...
    // 40 4HZ, 0.25s
    for (int j = i; j < 100 + i; j += 1) idx.push_back(j);
    ...
    for (int i : idx) {
      ...
      Eigen::aligned_map<int, Eigen::aligned_vector<pair<int, Eigen::Matrix<double, 7, 1>>>> image;
      ...
      if (pDrtVioInit->addFeatureCheckParallax(cur_img_t_s, image, 0.0)) {
        ...
        std::cout << "add image is: " << fixed << cur_img_t_s << " image number is: " << idx_time.size() << std::endl;
    ...
    if (!pDrtVioInit->process()) {
      ...
      save_file << "scale_error: "
                << "nan" << std::endl;
      save_file << "pose_error: "
                << "nan" << std::endl;
      save_file << "biasg_error: "
                << "nan" << std::endl;
      save_file << "velo_error: "
                << "nan" << std::endl;
      save_file << "gravity_error: "
                << "nan"
                << " "
                << "nan" << std::endl;
      save_file << "v0_error: "
                << "nan" << std::endl;
      save_file << "gt_vel_rot: "
                << "nan"
                << " "
                << "nan" << std::endl;
      LOG(INFO) << "---scale: ";
    ...
    for (int i = 0; i < idx_time.size() - 1; i++) {
      ...
      double error = std::acos(((qij_gt * qij_est.inverse()).toRotationMatrix().trace() - 1.0) / 2.0) * 180.0 / M_PI;
    ...
    double gravity_error = 180. * std::acos(pDrtVioInit->gravity.normalized().dot(gt_g_imu[0].normalized())) / EIGEN_PI;
    ...
    for (int i = 0; i < idx_time.size(); i++) {
      velo_norm_rmse +=
          (gt_vel[i].norm() - pDrtVioInit->velocity[i].norm()) * (gt_vel[i].norm() - pDrtVioInit->velocity[i].norm());

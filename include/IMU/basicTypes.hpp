...
class IMUCalibParam {
 public:
  IMUCalibParam(const Eigen::Matrix3d &Rbc,
                const Eigen::Vector3d &tbc,
                const double ng,
                const double na,
                const double ngw,
                const double naw)
...
class CameraPose {
 public:
  ...
  inline void set_vel_bias(const Eigen::Vector3d &vel, const IMUBias &bias) {
    vel_ = vel;
    bias_ = bias;
    velbgba_param_[0] = vel_.x();
    velbgba_param_[1] = vel_.y();
    velbgba_param_[2] = vel_.z();
    velbgba_param_[3] = bias_.bg_.x();
    velbgba_param_[4] = bias_.bg_.y();
    velbgba_param_[5] = bias_.bg_.z();
    velbgba_param_[6] = bias_.ba_.x();
    velbgba_param_[7] = bias_.ba_.y();
    velbgba_param_[8] = bias_.ba_.z();
  }
  ...
  inline Eigen::Vector3d world2cam(const Eigen::Vector3d &p3d_world) { return Tcw_ * p3d_world; }
...
struct IMUMeasurement {
 public:
  IMUMeasurement() {}
  IMUMeasurement(const double t, const Eigen::Vector3d &gyro, const Eigen::Vector3d &acc)
      : timestamp_(t), gyro_(gyro), acc_(acc) {}
  double timestamp_;

...
class Transformation {
 ...
 public:
  Transformation() : _t(Eigen::Vector3d::Zero()), _R(Eigen::Matrix3d::Identity()) {}

  Transformation(const Eigen::Vector3d &t, const Eigen::Matrix3d &R) : _t(t), _R(R) {}
  ...
  Transformation inverse() const { return Transformation(-_R.transpose() * _t, _R.transpose()); }

  Transformation operator*(const Transformation &T) const { return Transformation(_R * T._t + _t, _R * T._R); }

  Eigen::Vector3d operator*(const Eigen::Vector3d &v) const { return _R * v + _t; }
};
...
static double SampsonError(const Eigen::Matrix3d &E, const Eigen::Vector3d &f_0, const Eigen::Vector3d &f_1) {
...
static double SampsonError(const Eigen::Vector3d &t,
                           const Eigen::Matrix3d &R,
                           const Eigen::Vector3d &f_0,
                           const Eigen::Vector3d &f_1) {
...
static bool CheckPointInFrontOfViews(const Eigen::Vector3d &f_0,
                                     const Eigen::Vector3d &f_1,
...
static void DecomposeEssentialMatrix(const Eigen::Matrix3d &E,
                                     Eigen::Vector3d &t,
                                     Eigen::Matrix3d &R1,
                                     Eigen::Matrix3d &R2) {
  Eigen::Matrix3d W;
...
static int RecoverPoseFromEssentialMatrix(const Eigen::Matrix3d &E,
  ...
  std::vector<Transformation> T_vec = {
      Transformation(t, R1), Transformation(-t, R1), Transformation(t, R2), Transformation(-t, R2)};
  ...
  for (size_t i = 0; i < T_vec.size(); ++i) {
    ...
    for (size_t j = 0; j < f_0_vec.size(); ++j) {
      if (!inlier_flags.empty() && !inlier_flags[j]) continue;
      if (!CheckPointInFrontOfViews(f_0_vec[j], f_1_vec[j], T_vec[i])) continue;
      ++inlier_count_vec[i];
  ...
  if (!inlier_flags.empty()) {
    for (size_t i = 0; i < inlier_flags.size(); ++i) inlier_flags[i] = inlier_flags[i] && inlier_flags_vec[max_id][i];

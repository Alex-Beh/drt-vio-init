...
IMUPreintegrated::IMUPreintegrated(const IMUBias &bias,
...
void IMUPreintegrated::integrate_new_measurement(const Eigen::Vector3d &gyro_mea,
                                                 const Eigen::Vector3d &acc_mea,
                                                 const double &dt) {
  ...
  acc_skew << 0.0, -acc.z(), acc.y(), acc.z(), 0.0, -acc.x(), -acc.y(), acc.x(), 0.0;
...
Matrix15d IMUPreintegrated::get_information()  // TODO:: avoid re-computation.
{
  ...
  // std::cout << "compare informatrix:\n" << Info_.block<9,9>(0,0) << std::endl << Cov_.block<9,9>(0,0).inverse() <<
  // std::endl;
  return Info_;
}

...
/// @cite Christian Forster, Luca Carlone, Frank Dellaert, Davide Scaramuzza, “On-Manifold Preintegration for Real-Time
/// Visual-Inertial Odometry”, in IEEE Transactions on Robotics, 2016.
bool ImuIntegFactor::Evaluate(double const* const* parameters, double* residuals, double** jacobians) const {
  ...
  Eigen::Vector3d ev =
      invRi * (Vj - Vi - g * dt) -
      pre_integration_->get_delta_velocity(bias).cast<double>();  // eq.(45)  ev = Ri^{T}*(Vj-Vi-g*dt) - dV
  Eigen::Vector3d ep =
      invRi * (Pj - Pi - Vi * dt - 0.5 * g * dt * dt) -
  ...
  if (jacobians) {
    ...
    if (jacobians[0]) {
      ...
      jacobian_pose_i.block<3, 3>(6, 0) =
          Sophus::SO3d::hat(invRi * (Pj - Pi - Vi * dt - 0.5 * g * dt * dt));  // eq.(74)  dep/dphi
      // translation
      ...
      // jacobian check
      {
        ...
        //                     Sophus::SO3d erSO3_new = (pre_integration_->get_delta_rotation(bias).inverse()) *
        //                     Ri_new.inverse() * Rj; Eigen::Vector3d er_new = erSO3_new.log(); //
        //                     // eq.(45)  er = Log(dR^{T} * Rwbi^{T} * Rwbj)
        //                     Eigen::Vector3d ev_new = Ri_new.inverse()*(Vj-Vi-g*dt) -
        //                     pre_integration_->get_delta_velocity(bias);
        //                     // eq.(45)  ev = Ri^{T}*(Vj-Vi-g*dt) - dV
        //                     Eigen::Vector3d ep_new = Ri_new.inverse()*(Pj - Pi_new - Vi*dt - 0.5*g*dt*dt) -
        //                     pre_integration_->get_delta_position(bias); //
    ...
    if (jacobians[1]) {
      ...
      // jacobian check
      {
        ...
        //                     Sophus::SO3d erSO3_new = pre_integration_->get_delta_rotation(bias_new).inverse()*invRi*
        //                     Rj; Eigen::Vector3d er_new = erSO3_new.log(); //
        //                     // eq.(45)  er = Log(dR^{T} * Rwbi^{T} * Rwbj)
        //                     Eigen::Vector3d ev_new = invRi*(Vj-Vi_new-g*dt) -
        //                     pre_integration_->get_delta_velocity(bias_new);
        //                     // eq.(45)  ev = Ri^{T}*(Vj-Vi-g*dt) - dV
        //                     Eigen::Vector3d ep_new = invRi*(Pj - Pi - Vi_new*dt - 0.5*g*dt*dt) -
        //                     pre_integration_->get_delta_position(bias_new);
        //                     // eq.(45)  ep = Ri^{T}*(Pj-Pi-Vi*dt-0.5*g*dt^2) - dP
...
bool ImuIntegGdirFactor::Evaluate(double const* const* parameters, double* residuals, double** jacobians) const {
  ...
  Eigen::Vector3d er = erSO3.log();  // eq.(45)  er = Log(dR^{T} * Rwbi^{T} * Rwbj)
  Eigen::Vector3d ev =
      invRi * (Vj - Vi - g_new * dt) -
      pre_integration_->get_delta_velocity(bias).cast<double>();  // eq.(45)  ev = Ri^{T}*(Vj-Vi-g*dt) - dV
  Eigen::Vector3d ep =
      invRi * (Pj - Pi - Vi * dt - 0.5 * g_new * dt * dt) -
      pre_integration_->get_delta_position(bias).cast<double>();  // eq.(45)  ep = Ri^{T}*(Pj-Pi-Vi*dt-0.5*g*dt^2) - dP
  Eigen::Vector3d ebg = Bgj - Bgi;
  Eigen::Vector3d eba = Baj - Bai;
  Eigen::Map<Eigen::Matrix<double, 15, 1>> residual(residuals);
  ...
  if (jacobians) {
    ...
    if (jacobians[0]) {
      ...
      jacobian_pose_i.block<3, 3>(6, 0) =
          Sophus::SO3d::hat(invRi * (Pj - Pi - Vi * dt - 0.5 * g_new * dt * dt));  // eq.(74)  dep/dphi
      // translation
      jacobian_pose_i.block<3, 3>(6, 3) = -Eigen::Matrix3d::Identity();  // eq.(71)  dep/dp
      ...
      // jacobian check
      {
        ...
        //     Eigen::Vector3f er_new = erSO3_new.log(); // eq.(45)  er = Log(dR^{T} * Rwbi^{T} * Rwbj) Eigen::Vector3f
        //     ev_new = Ri_new.inverse()*(Vj-Vi-g*dt) - pre_integration_->get_delta_velocity(bias); // eq.(45)  ev =
        //     Ri^{T}*(Vj-Vi-g*dt) - dV Eigen::Vector3f ep_new = Ri_new.inverse()*(Pj - Pi_new.cast<float>() - Vi*dt -
        //     0.5*g*dt*dt) - pre_integration_->get_delta_position(bias); // eq.(45)  ep =
        //     Ri^{T}*(Pj-Pi-Vi*dt-0.5*g*dt^2) - dP Eigen::Vector3f ebg_new = Bgj - Bgi; Eigen::Vector3f eba_new = Baj -
        //     Bai;
    ...
    if (jacobians[1]) {
      ...
      // jacobian check
      {
        ...
        //     Eigen::Vector3f er_new = erSO3_new.log(); // eq.(45)  er = Log(dR^{T} * Rwbi^{T} * Rwbj) Eigen::Vector3f
        //     ev_new = invRi*(Vj-Vi_new.cast<float>()-g*dt) - pre_integration_->get_delta_velocity(bias_new); //
        //     eq.(45)  ev = Ri^{T}*(Vj-Vi-g*dt) - dV Eigen::Vector3f ep_new = invRi*(Pj - Pi - Vi_new.cast<float>()*dt
        //     - 0.5*g*dt*dt) - pre_integration_->get_delta_position(bias_new);       // eq.(45)  ep =
        //     Ri^{T}*(Pj-Pi-Vi*dt-0.5*g*dt^2) - dP Eigen::Vector3f ebg_new = Bgj - Bgi_new.cast<float>();
        //     Eigen::Vector3f eba_new = Baj - Bai_new.cast<float>();

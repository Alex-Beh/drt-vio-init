...
namespace DRT {
drtLooselyCoupled::drtLooselyCoupled(const Eigen::Matrix3d &Rbc, const Eigen::Vector3d &pbc) : drtVioInit(Rbc, pbc) {}
...
bool drtLooselyCoupled::process() {
  ...
  if (!gyroBiasEstimator()) return false;
  ...
  for (const auto &pt : SFMConstruct) {
    ...
    for (const auto &frame : obs) {
      ...
      if (i_view_id != lbase_view_id) {
        ...
        Eigen::Matrix3d R_cicl = frame_rot.at(i_view_id).transpose() * frame_rot.at(lbase_view_id);

        Eigen::Matrix3d R_crcl = frame_rot.at(rbase_view_id).transpose() * frame_rot.at(lbase_view_id);

        Eigen::Vector3d a_lr_tmp_t =
            cross_product_matrix(R_crcl * obs.at(lbase_view_id).normalpoint) * obs.at(rbase_view_id).normalpoint;

        Eigen::RowVector3d a_lr_t = a_lr_tmp_t.transpose() * cross_product_matrix(obs.at(rbase_view_id).normalpoint);
        ...
        A_lr.row(track_id).block<1, 3>(0, time_frameid2_int_frameid.at(rbase_view_id) * 3) =
            -a_lr_t * frame_rot.at(rbase_view_id).transpose();
        // theta_lr
        Eigen::Vector3d theta_lr_vector =
            cross_product_matrix(obs.at(rbase_view_id).normalpoint) * R_crcl * obs.at(lbase_view_id).normalpoint;
        ...
        Eigen::Matrix3d Coefficient_B =
            xi_cross * R_cicl * obs.at(lbase_view_id).normalpoint * a_lr_t * frame_rot.at(rbase_view_id).transpose();

        // calculate matrix C [i_view_id]
        Eigen::Matrix3d Coefficient_C =
            theta_lr * cross_product_matrix(obs.at(i_view_id).normalpoint) * frame_rot.at(i_view_id).transpose();
        ...
        // #pragma omp critical
        {
          if (time_frameid2_int_frameid.at(lbase_view_id) > 0)
            LTL.middleRows<3>(time_frameid2_int_frameid.at(lbase_view_id) * 3 - 3) +=
                LTL_l_row.rightCols(LTL_l_row.cols() - 3);

          if (time_frameid2_int_frameid.at(rbase_view_id) > 0)
            LTL.middleRows<3>(time_frameid2_int_frameid.at(rbase_view_id) * 3 - 3) +=
                LTL_r_row.rightCols(LTL_r_row.cols() - 3);

          if (time_frameid2_int_frameid.at(i_view_id) > 0)
            LTL.middleRows<3>(time_frameid2_int_frameid.at(i_view_id) * 3 - 3) +=
                LTL_i_row.rightCols(LTL_i_row.cols() - 3);
        }
      }
    }

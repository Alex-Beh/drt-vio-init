...
drtTightlyCoupled::drtTightlyCoupled(const Eigen::Matrix3d &Rbc, const Eigen::Vector3d &pbc) : drtVioInit(Rbc, pbc) {}
...
bool drtTightlyCoupled::process() {
  ...
  for (int i = 0; i < imu_meas.size(); i++) {
    accum_imu_meas.insert(
        accum_imu_meas.end(), imu_meas[i].imu_measurements_.begin(), imu_meas[i].imu_measurements_.end());
  ...
  for (const auto &pt : SFMConstruct) {
    ...
    for (const auto &frame : obs) {
      ...
      if (i_view_id != rbase_view_id) {
        ...
        A_tmp.block(0, 3, 3, 3) =
            -(B_prime * t_1r * t_1r / 2. + C_prime * t_1i * t_1i / 2. + D_prime * t_1l * t_1l / 2.) * G.norm();

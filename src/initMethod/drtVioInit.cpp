...
void drtVioInit::addImuMeasure(const vio::IMUPreintegrated &imuData) { imu_meas.push_back(imuData); }

bool drtVioInit::addFeatureCheckParallax(TimeFrameId frame_id, const FeatureTrackerResulst &image, double td) {
  ...
  if (local_active_frames.size() == 0) {
    ...
    for (const auto &pts : image) {
      if (SFMConstruct.find(pts.first) != SFMConstruct.end()) {
        Eigen::Vector3d cur_pt{pts.second[0].second(0), pts.second[0].second(1), pts.second[0].second(2)};
...
bool drtVioInit::checkAccError() {
  ...
  for (int i = 0; i < imu_meas.size(); i++) {
    ...
    if (std::abs(acc.norm() - G.norm()) / G.norm() < 5e-3) is_bad[i] = 1;
  ...
  if (avgA_error > 5e-3 and scoreSum <= 1) check_success = true;
...
bool drtVioInit::gravityRefine(const Eigen::MatrixXd &M,
  ...
  const double t2 = s00 * s11 + s00 * s22 + s11 * s22 - std::pow(s01, 2) - std::pow(s02, 2) - std::pow(s12, 2);
  const double t3 =
      s00 * s11 * s22 + 2. * s01 * s02 * s12 - s00 * std::pow(s12, 2) - s11 * std::pow(s02, 2) - s22 * std::pow(s01, 2);

  Eigen::VectorXd coeffs(7);
  coeffs << 64., 64. * t1, 16. * (std::pow(t1, 2) + 2. * t2), 16. * (t1 * t2 + t3),
      4. * (std::pow(t2, 2) + 2. * t1 * t3), 4. * t3 * t2, std::pow(t3, 2);
  ...
  if (constraint < 0. || std::abs(std::sqrt(constraint) - gravity_mag) / gravity_mag > 1e-3) {  // TODO
    LOG(WARNING) << "Discarding bad solution...\n";
    printf("constraint: %.16f\n", constraint);
    printf("constraint error: %.2f\n", 100. * std::abs(std::sqrt(constraint) - gravity_mag) / gravity_mag);
    return false;
...
bool drtVioInit::gyroBiasEstimator() {
  ...
  for (int i = 0; i < int_frameid2_time_frameid.size() - 1; i++) {
    ...
    ceres::CostFunction *eigensolver_cost_function =
        BiasSolverCostFunctor::Create(fis, fjs, Eigen::Quaterniond(Rbc_), imu1);

...
void GetSwOpenGLPoseMatrices(std::vector<pangolin::OpenGlMatrix>& vPoses_SW,
                             const std::vector<Eigen::Isometry3d>& swPoses) {
  vPoses_SW.clear();
...
void DrawActiveMapPoints(const std::map<int, Eigen::Vector3d>& mMapPoints) {
  if (mMapPoints.empty()) return;

  glPointSize(5);
  glBegin(GL_POINTS);
  glColor3f(1.0, 0.0, 0.0);

  for (auto pt : mMapPoints) glVertex3f((pt.second)(0), (pt.second)(1), (pt.second)(2));
